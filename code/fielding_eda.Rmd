...
plot_histogram <- function(var, df,  bw = 1, x_max = NULL) {
  ...
  ggplot(aes_string(x = toString(var)), data = df) +
    ...
    scale_x_continuous(breaks = seq(0, x_max, bw), minor_breaks = NULL, expand = c(0,0)) +
...
fluidPage(
  ...
  fluidRow(
    ...
    column(4, selectInput("f_stat", label = "Select fielding stat", choices = fielding_stats, selected = "assists"),
           verbatimTextOutput("pop_plot_hover")),
    ...
    column(4, br(), sliderInput("bin_width", label = "Select Bin Width", min = 1, max = 100, value = c(10), step = 1))
  ),
  
  fluidRow(
    
    column(12, plotOutput("pop_plot", hover = hoverOpts(id = "pop_hover")), 
    
    #column(width = 3, verbatimTextOutput("pop_plot_hover")),
...
output$pop_plot_hover <- renderPrint(str(input$pop_hover$x + (input$bin_width / 2)))

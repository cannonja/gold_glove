...
plot_histogram <- function(var, df,  bw = 1) {
  ...
  ggplot(aes_string(x = toString(var)), data = df) +
    ...
    scale_x_continuous(breaks = seq(0, max(df[toString(var)]), bw), minor_breaks = NULL) +
...
renderPlot({
  ...
  plot_histogram(input$f_stat, df_in[df_in$won_gg == 1,], input$bin_width)
